...
def main():
    ...
    if args.voxelfe:
        ...
        if args.mapping is None:
            ...
            voxelFE.vol2voxelfe(L, args.template, fileout_base + "_voxelFE.inp", keywords=['NSET', 'ELSET'], voxelsize=sp, refnode=args.refnode, verbose=args.verbose)
            ...
            voxelFE.vol2voxelfe(masked, args.template, fileout_base + "_voxelFE.inp", matprop, keywords=['NSET', 'ELSET', 'PROPERTY'], voxelsize=sp, verbose=args.verbose)
    ...
    if args.tetrafe:
        ...
        if args.mapping is None:
            if 'volmesh' not in locals():
                # check CGAL parameters
                max_facet_distance, max_cell_circumradius = tetraFE.ccheck_cgal_params(args.max_facet_distance, args.max_cell_circumradius, sp)
            ...
            tetraFE.cmesh2tetrafe(volmesh, args.template, fileout_base + "_tetraFE.inp", keywords=['NSET', 'ELSET'], verbose=args.verbose)
